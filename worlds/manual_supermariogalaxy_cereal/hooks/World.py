...
def before_create_items_starting(item_pool: list, world: World, multiworld: MultiWorld, player: int) -> list:
    ...
    # Movement Randomization OFF → give all Moves
    if not world.options.Movement_Randomization:
        for item in item_pool:
            if "Moves" in world.item_name_to_item[item.name].get("category", []):
                multiworld.push_precollected(item)
            else:
                new_pool.append(item)
    else:
        new_pool = item_pool.copy()

    item_pool = new_pool
    new_pool = []

    # Mechanic Randomization OFF → give all Mechanics
    if not world.options.Mechanic_Randomization:
        for item in item_pool:
            if "Mechanics" in world.item_name_to_item[item.name].get("category", []):
                multiworld.push_precollected(item)
            else:
                new_pool.append(item)
    else:
